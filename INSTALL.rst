...
``src/importer/conf/local.py``. You can base this file on the
...
.. code-block:: bash
    ...
    $ python src/manage.py test importer
...
* ``DB_NAME``: name of the database for the project. Defaults to ``catalogi-importer``.
* ``DB_USER``: username to connect to the database with. Defaults to ``catalogi-importer``.
* ``DB_PASSWORD``: password to use to connect to the database. Defaults to ``catalogi-importer``.
...
1. Clone or download the code from `Github`_ in a folder like
   ...
   .. code-block:: bash
       ...
       $ git clone git@github.com:maykinmedia/catalogi-importer.git
       Cloning into 'catalogi-importer'...
       ...
...
.. _Github: https://github.com/maykinmedia/catalogi-importer/
...
environment variables. See ``src/importer/conf/docker.py`` for
all settings.

.. code-block:: bash

    $ docker build -t catalogi-importer
    $ docker run \
        -p 8000:8000 \
        -e DATABASE_USERNAME=... \
        -e DATABASE_PASSWORD=... \
    ...
    $ docker exec -it catalogi-importer /app/src/manage.py createsuperuser
